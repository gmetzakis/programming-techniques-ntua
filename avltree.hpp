...
//    Thn count thn exw valei anamesa se polla hashtags("#") gia na fainetai. (ln 214)
//    Meta exw prosthesei sthn "struct node" apla to "int pollaplothta"
//    enw stis methodous "insert" kai "remove" exw prosthesei diafora pou ekshgw ekei leptomerws.

//    Gia ola ta dika mou comments exw xrhsimopoihsei greeklish gia na
//    ksexwrizoun apo ta hdh iparxonta.
...
private:
  ...
  struct node {
    T data;
    int pollaplothta;       //prosthetw to pedio afto gia na apothikevw thn pollaplothta
    balance_type balance;
    node *left, *right, *parent;

    node(const T &x, node *p = nullptr)
        : data(x), pollaplothta(x), balance(EH), left(nullptr), right(nullptr), parent(p) {}
  };
...
public:
  ...
  void insert(const T &x) {
    ...
    if (root == nullptr) {
      root = new node(x);
      ++the_size;
      root->pollaplothta = 1;   //otan kanw insert thn root ousiastika, profanws h pollpalothta
    }                           //einai 1, afou den iparxei prin.
    else { 
      int flag1 = 0;            //h flag1 bainei gia na diakrinw to an eisagetai diplotypo h oxi.
      if (lookup(root,x) != nullptr) {  //an DEN einai nullptr, dhladh epistrepsei kapoion komvo,
        flag1 = 1; }            //thetw thn flag1 = 1, wste na kserw oti exw diplotypo.
      node *p = insert(root, x);
      if (p != nullptr) {
        ++the_size;
        if (flag1 == 0)     //an flag1==0 tote bainei 1h fora afto to kleidi, ara arxikopoiw
          p->pollaplothta = 1;  //thn pollaplothta tou na einai ish me 1.
        else {
        lookup(root,x)->pollaplothta++; //alliws afksanw kata 1 thn pollaplothta tou komvou
        p->pollaplothta = lookup(root,x)->pollaplothta; //pou epestrepse h lookup, kai meta thn 
        }                       //vazw kai sthn pollaplothta tou neoeiserxomenou komvou.
...
private:
  ...
  static node *insert(node *t, const T &x) {
    while (true) {
      ...
      else {        //edw bainw otan exw duplicate ousiastika, afou x == t->data
        if (t->left == nullptr) {
          return (t->left = new node(x, t));
        }           //telikws, tha ginei insert opou vrei komvo opou DEN exei 2 paidia
        else if (t->right == nullptr) {
          return (t->right = new node(x,t));
        }
        return insert(t->left,x);   //alliws tha psaksei pros ta aristera mexri na vrei tetoion
      }
    }
  }
...
public: 
  int count(const T &x) {
    int q;
    if (lookup(root,x) != nullptr) {    //an vrei komvo me tetoia timh
      node *rr = lookup(root,x);        //tha metaferthei sthn private "count" opou epistrefei
      q = count(rr,x);                  //thn pollaplothta tou kleidiou aftou sthn int metavlith "q"
    }
    else q=0;               //alliws, den iparxei komvos me afto to kleidi, ara thetw "q=0"
    return q;               //kai epistrefw to apotelesma ths zitoumenhs sinarthshs "count"
  }

private:
  int count(node *t, const T &x) {
    int y = t->pollaplothta;        //edw apla epistrefei thn timh tou pediou tou komvou opou
    return y;                       //apothikevetai h pollaplothta tou
  } 
...
public:
  ...
  bool remove(const T& x) {
    ...
    t->pollaplothta--;      //meiwnw thn pollaplothta tou afairoumenou komvou
    --the_size;
    int poll1 = t->pollaplothta;    //kai thn apothikevw sthn metavlith "poll1"
    remove(t);
    delete t;
    node *r = lookup(root,x);   //afotou afairethei, psaxnw na dw an h lookup mou
    if (r != nullptr) r->pollaplothta = poll1;  //epistrefei allon komvo me idio data
    return true;    //kai an epistrefei, enhmerwnw thn pollaplothta tou, me to "poll1"
  }
